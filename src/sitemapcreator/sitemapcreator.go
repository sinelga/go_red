...
func Createsitemap(golog syslog.Writer, locale string, themes string, keyword_phrasearr []domains.Keyword_phrase, site string) []byte {
...
//		pubdate := time.Date(d.Year(), d.Month(), d.Day()-(i+1), 0, 0, 0, 0, time.UTC).Local().Format(time.RFC3339)
		...
		if locale == "fi_FI" && themes == "fortune" {
			...
			pubdate = time.Date(d.Year(), d.Month(), d.Day()-1, 0, 0, 0, 0, time.UTC).Local().Format(time.RFC3339)
